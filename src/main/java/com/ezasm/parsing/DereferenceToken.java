...
public class DereferenceToken extends RightHandToken {
    ...
    /**
     * Constructs a dereference token based on the given text. Assumes that the given text has already
     * been validated to be a dereference. The text must begin with an immediate value. There must then
     * be a statement comprised of a register surrounded by parenthesis i.e. "($sp)". An example of a
     * valid token text would be "-4($sp)".
     *
